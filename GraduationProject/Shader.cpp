...
void CDepthRenderShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, void* pContext)
{
	...
	m_pDepthTexture = new CTexture(MAX_DEPTH_TEXTURES, RESOURCE_TEXTURE2D_ARRAY, 0, 1,0,0);
...
{
	if (m_pDepthTexture) m_pDepthTexture->UpdateGraphicsShaderVariables(pd3dCommandList);
}
